...
-   **Provider Subscriptions:** Boost (₹99/₹999), Pro (₹199/₹1999), Premium (₹499/₹4999) plans on monthly/yearly cycles via Cashfree. Plans grant search-result pinning and tick badges (white/yellow/green via `PlanTick`). Charging rules in `chargeAndCreateCall`: provider Premium = both sides 0; provider Pro = caller 1, provider 0; caller Premium = caller side 0; Pro/Premium providers bypass the daily call cap; 0-credit calls still create rows (no insufficient_balance throw). Cashfree `verify-payment` is idempotent per `order_id` and validates the paid amount against the live `subscription_plans` config before extending. Same-tier purchases extend `endDate`; cross-tier purchases cancel the old plan and the new tier starts fresh from `now` (remaining time on the old tier is forfeited). Active subscriptions whose `endDate` has passed are lazily transitioned to `expired` on every read path. Admin "Plans" tab edits prices/perks, lists all subscriptions with filters, and supports grant/cancel/extend.
