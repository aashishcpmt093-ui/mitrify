...
export const INDIA_STATES_DISTRICTS: StateDistricts[] = [
  ...
  },
  {
    ...
    districts: [
      ...
      "NTR", "Nandyal", "Nellore", "Palnadu", "Prakasam",
      "Sri Balaji", "Sri Sathya Sai", "Srikakulam", "Visakhapatnam",
  ...
  },
  {
    ...
    districts: [
      ...
      "Koriya", "Mahasamund", "Manendragarh", "Mohla-Manpur",
...
export const ALL_STATES = INDIA_STATES_DISTRICTS.map(s => s.state);

export function getDistricts(state: string): string[] {
  const found = INDIA_STATES_DISTRICTS.find(s => s.state === state);
  return found ? found.districts : [];
}

export const DEFAULT_STATE = "Uttar Pradesh";
export const DEFAULT_DISTRICT = "Budaun";
