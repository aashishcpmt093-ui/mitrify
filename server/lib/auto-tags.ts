...
async function callGemini(serviceName: string, description: string | null): Promise<string[]> {
  ...
  // gemini-flash-latest = Gemini 2.5 Flash, which has "thinking mode" ON by
  // default. With thinking on, the model spends part (often ALL) of its
  // output budget on internal `thoughtsTokenCount` and the actual reply
  // text comes back empty / truncated with `finishReason: MAX_TOKENS`.
  // We disable thinking explicitly (thinkingBudget: 0) so the full output
  // budget is available for the actual JSON. Also bump maxOutputTokens to
  // 512 defensively — 8-10 hyphenated tags fit easily but the API counts
  // JSON syntax tokens too.
  const body = {
  ...
  if (!text) {
    // Distinguish "model returned literally nothing" from the common
    // thinking-mode failure mode where MAX_TOKENS is hit before any reply
    // text is emitted. Throw so the caller can record it as an AI error
    // instead of silently falling back to dictionary.
    if (finishReason === "MAX_TOKENS") {
      throw new Error("empty response (MAX_TOKENS — likely thinking-mode budget exhausted)");
    }
    throw new Error(`empty response${finishReason ? ` (finishReason: ${finishReason})` : ""}`);
  }
  ...
  if (!Array.isArray(arr)) {
    throw new Error(`parse failed (text starts: "${text.slice(0, 60).replace(/\s+/g, " ")}")`);
  }
  const tags = arr.filter((x: any) => typeof x === "string");
  if (tags.length === 0) {
...
export interface TagResult {
  ...
  newTagsAdded: number;
  source: TagSource;
  /** Set when a Gemini API call was attempted and failed (network error,
   *  HTTP error, MAX_TOKENS, parse failure, empty array). The job runner
   *  uses this to count AI-specific failures (vs. generic worker errors)
   *  and surface a banner to the admin so silent Gemini outages don't
   *  silently fall back to dictionary forever. */
  aiError?: string;
}
...
export async function generateTagsForProvider(
  ...
  if (dict.length >= MIN_TAGS_BEFORE_AI) {
    ...
    try {
      ...
      // callGemini now throws on every empty/parse-fail path, so reaching
      // here means we have a non-empty array.
      source = dict.length > 0 ? "hybrid" : "ai";
    } catch (err: any) {
      aiError = err?.message || String(err);
      console.warn("[auto-tags] Gemini call failed:", aiError);
  ...
  return { finalTags: merged, newTagsAdded: added, source, aiError };
}
...
export interface JobStatus {
  ...
  skipped: number;
  failed: number;
  /** Counts providers where the Gemini API call failed (network, HTTP,
   *  MAX_TOKENS, parse failure). Distinct from `failed` (generic worker
   *  exceptions) and `skipped` (no useful tags but no error either). */
  aiErrors: number;
  done: boolean;
  startedAt: number;
  finishedAt?: number;
    ...
    const worker = async () => {
      while (true) {
        ...
        try {
          ...
          // Track Gemini-side failures separately. Even when we recovered
          // gracefully (dict fallback), we want the admin to see that AI
          // is broken instead of assuming everything's fine because the
          // overall job ran.
          if (result.aiError) {
            status.aiErrors++;
