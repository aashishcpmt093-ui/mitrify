...
export async function ensureSchema(): Promise<void> {
  ...
  try {
    ...
    // ── STEP 0: Base tables from migration 0000 ───────────────────────────────
    // On a brand-new database (fresh Railway Postgres) nothing exists yet, so
    // the base tables must be created here too — otherwise STEP 2's indexes on
    // "profiles" throw and abort the whole repair. Definitions are verbatim
    // from migrations/0000_same_loners.sql; STEPS 3-5 below then bring them up
    // to the current shape (extra columns, partial unique indexes, renames).
